...
export default function Home() {
  ...
  return (
    ...
    >
              ...
              return (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-[10px] md:text-xs font-bold tracking-wider uppercase transition-all duration-300 ${
                    isActive 
                      ? "bg-primary-green text-white shadow-lg shadow-primary-green/30 scale-105" 
      ...
      <style jsx global>{`
        :root {
          --primary-green: ${pColor};
          --color-primary-green: ${pColor};
      ...
      <div className="fixed bottom-8 right-8 md:bottom-12 md:right-16 z-[300] flex flex-col items-end gap-4">
        ...
        >
          <Bell size={24} />
          {unreadCount > 0 && <span className="absolute -top-1 -right-1 bg-primary-green text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center border-2 border-white/10">{unreadCount}</span>}
        </motion.button>
        ...
        <motion.button onClick={() => setIsContactOpen(true)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="bg-primary-green text-white w-14 h-14 md:w-16 md:h-16 rounded-full shadow-2xl flex items-center justify-center group overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-[#0a0a0a]/90 backdrop-blur-2xl border border-white/20 rounded-3xl shadow-2xl p-10 text-white">
              <form onSubmit={handleAuth} className="space-y-8">
                <div className="space-y-2 text-center">
                  <h3 className="font-serif text-4xl italic text-primary-green">{isSignUp ? "Créer un compte" : "Connexion"}</h3>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <input type="email" value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm outline-none focus:border-primary-green transition-all" placeholder="votre@email.com" required />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-white/30 ml-1">Mot de passe</label>
                    <input type="password" value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm outline-none focus:border-primary-green transition-all" placeholder="••••••••" required />
                  </div>
                </div>

                {authError && <p className="text-red-500 text-[10px] font-bold uppercase text-center tracking-widest">{authError}</p>}

                <button type="submit" disabled={isAuthLoading} className="w-full bg-primary-green text-white py-5 rounded-2xl font-bold text-xs tracking-widest uppercase hover:bg-red-600 transition-all shadow-2xl shadow-primary-green/30 flex items-center justify-center gap-3">
                ...
                <p className="text-center text-[10px] text-white/40 uppercase tracking-widest">
                  ...
                  <button type="button" onClick={() => setIsSignUp(!isSignUp)} className="ml-2 text-primary-green hover:underline font-bold">
                ...
                {selectedProduct.images.length > 1 && (
                  <>
                    <button 
                      ...
                      className="absolute left-6 top-1/2 -translate-y-1/2 z-20 p-3 bg-black/40 hover:bg-primary-green rounded-full text-white backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 hidden md:flex"
                    >
                      <ArrowLeft size={20} />
                    </button>
                    <button 
                      onClick={(e) => { e.stopPropagation(); setActiveProdImg(prev => (prev + 1) % selectedProduct.images.length); }}
                      className="absolute right-6 top-1/2 -translate-y-1/2 z-20 p-3 bg-black/40 hover:bg-primary-green rounded-full text-white backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 hidden md:flex"
                    >
                    ...
                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 z-20">
                      ...
                      <div className="flex gap-2">
                        {selectedProduct.images.map((_, i) => (
                          <button key={i} onClick={() => setActiveProdImg(i)} className={`w-2 h-2 rounded-full transition-all ${activeProdImg === i ? 'bg-primary-green w-4' : 'bg-white/20 hover:bg-white/40'}`} />
                        ))}
                      </div>
                    </div>
                  </>
              ...
              <div className="w-full md:w-2/5 p-8 md:p-16 flex flex-col justify-center bg-black/40 backdrop-blur-3xl overflow-y-auto">
                <div className="space-y-8">
                  <div className="space-y-4">
                    ...
                    <p className="text-4xl font-serif italic text-primary-green">{selectedProduct.price}€</p>
                  </div>

                  <div className="h-[1px] w-20 bg-primary-green/30"></div>
                  ...
                  <div className="space-y-6">
                    ...
                    {selectedProduct.link && (
                      <div className="pt-4">
                        <Link 
                          ...
                          target="_blank"
                          className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-primary-green border-b border-primary-green/30 pb-1 hover:border-primary-green transition-all"
                        >
                   ...
                   <div className="pt-8 space-y-6">
                    <button 
                      ...
                      className="w-full bg-primary-green text-white py-6 rounded-2xl font-bold text-xs tracking-widest uppercase hover:bg-primary-green/80 transition-all shadow-2xl shadow-primary-green/30 flex items-center justify-center gap-4 group"
                    >
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {r.order_id && <span className="text-[9px] font-bold text-primary-green">Code: {r.order_id}</span>}
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4 mb-4">
                      {r.replies && r.replies.length > 0 ? (
                        r.replies.map((rep: { text: string; date: string; from: string; media?: { url: string; type: string }[] }, idx: number) => (
                          <div key={idx} className={`${rep.from === 'Lucas' ? 'bg-white/5 border-l-2 border-primary-green pl-4 py-2 rounded-r-lg' : ''} space-y-3`}>
                            <div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {r.reply && (
                        <button 
                          ...
                          className="text-[10px] font-bold text-primary-green uppercase tracking-widest hover:underline flex items-center gap-1"
                        >
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div className="overflow-hidden">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-primary-green truncate">Connecté en tant que</p>
                ...
                <div className="space-y-4 pt-4 border-t border-white/5">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-[10px] outline-none focus:border-primary-green transition-all text-white font-medium"
                    />
                  </div>
                  ...
                  <div className="space-y-3">
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-[10px] outline-none focus:border-primary-green transition-all text-white"
                    />
                    
                    <div className="flex items-center justify-between pt-1">
                      <span className="text-[8px] font-bold uppercase tracking-widest text-white/30">Ou local</span>
                      <label className="cursor-pointer text-[10px] font-bold text-primary-green uppercase tracking-widest flex items-center gap-1.5 hover:text-red-400 transition-colors">
                  ...
                  {user.email === 'caillatlucas2304@gmail.com' && (
                    <div className="space-y-4 pt-4 border-t border-white/5">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-primary-green">Paramètres du Site (Lucas)</p>
                      ...
                      <div className="space-y-2">
                        ...
                        <textarea 
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-[10px] outline-none focus:border-primary-green transition-all text-white font-medium resize-none"
                        />
                      </div>
                    </div>
                  )}
              ...
              <button 
                ...
                className="w-full bg-white/5 border border-white/10 text-white/60 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-primary-green hover:text-white transition-all flex items-center justify-center gap-2"
              >
            ...
            <motion.div initial={{ opacity: 0, y: 50, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 50, scale: 0.9 }} className="relative w-full max-w-lg bg-[#0a0a0a]/90 backdrop-blur-2xl border border-white/20 rounded-3xl shadow-2xl p-8 md:p-12 overflow-hidden text-white">
              {showSuccess ? (
                <div className="py-12 text-center space-y-6">
                  ...
                  {formOrderId && (
                    <div className="bg-primary-green/5 p-6 rounded-sm border border-primary-green/20 space-y-3">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-primary-green">Votre code de commande</p>
                ...
                <form onSubmit={handleContactSubmit} className="space-y-6">
                  <h3 className="font-serif text-4xl italic text-[var(--primary-green)]">Me contacter</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="Nom & Prénom" className="w-full bg-white/5 border-b border-white/20 py-3 outline-none focus:border-primary-green transition-all" required />
                    <input type="text" value={formContact} onChange={(e) => setFormContact(e.target.value)} placeholder="Email / Tél" className="w-full bg-white/5 border-b border-white/20 py-3 outline-none focus:border-primary-green transition-all" required />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <input type="text" value={formTitle} onChange={(e) => setFormTitle(e.target.value)} placeholder="Objet" className="w-full bg-white/5 border-b border-white/20 py-3 outline-none focus:border-primary-green transition-all" required />
                    <input type="text" value={formOrderId} onChange={(e) => setFormOrderId(e.target.value)} placeholder="ID Commande (Optionnel)" className="w-full bg-white/5 border-b border-white/20 py-3 outline-none focus:border-primary-green transition-all" readOnly={formTitle.startsWith("Achat:")} />
                  </div>
                  <textarea value={formContent} onChange={(e) => setFormContent(e.target.value)} placeholder="Message..." rows={4} className="w-full bg-white/5 border border-white/10 p-4 rounded-xl outline-none focus:border-primary-green resize-none transition-all" required />
                  
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] font-bold uppercase tracking-widest opacity-40">Pièces jointes</label>
                      <label className="cursor-pointer text-[10px] font-bold text-primary-green uppercase tracking-widest flex items-center gap-2">
                  ...
                  {(formOrderId || formTitle.startsWith("Achat:")) && (
                    <div className="flex items-start gap-3 bg-primary-green/5 p-4 rounded-sm border border-primary-green/10">
                      <input 
                        type="checkbox" 
                        id="orderAgree" 
                        checked={orderAgreed} 
                        onChange={(e) => setOrderAgreed(e.target.checked)} 
                        className="mt-1 accent-primary-green w-4 h-4 rounded-xs border-text-black/20" 
                        required 
                      />
                      <label htmlFor="orderAgree" className="text-[11px] leading-relaxed opacity-70 cursor-pointer">
                        Je m'engage à régler le montant de ce produit ({selectedProduct ? `${selectedProduct.price}€` : "indiqué"}) une fois la commande validée par Lucas. <span className="text-primary-green font-bold">*</span>
                      </label>
                    </div>
                  )}
            ...
            >
              ...
              <div className="hidden md:flex bg-[#0c0c0c]/90 backdrop-blur-2xl border border-white/15 p-5 rounded-[32px] w-72 flex-col shadow-[inset_0_1px_1px_rgba(255,255,255,0.15),0_20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden">
                ...
                <div className="space-y-1.5 mt-5">
                  ...
                  >
                    <div 
                      className="absolute left-0 top-0 h-full bg-primary-green rounded-full shadow-[0_0_8px_var(--primary-green)]" 
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <span className="text-[7px] font-bold uppercase tracking-[0.2em] text-primary-green leading-none mb-0.5">Musique</span>
                ...
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-white/10 overflow-hidden">
                  <div className="h-full bg-primary-green transition-all duration-300" style={{ width: `${playerProgress}%` }} />
                </div>
              </div>
            ...
            <section key="shop" id="shop" className="relative z-10 scroll-mt-36 md:scroll-mt-44">
              <div className="flex justify-center md:justify-start mb-16">
                <div className="inline-flex items-center gap-4 bg-[#0c0c0c]/85 backdrop-blur-2xl border border-white/15 px-6 py-3.5 rounded-full shadow-[inset_0_1px_1px_rgba(255,255,255,0.15),0_12px_40px_rgba(0,0,0,0.6)] transition-all duration-300">
                  <span className="w-2.5 h-2.5 bg-primary-green rounded-full animate-pulse shadow-[0_0_10px_var(--primary-green)]"></span>
                ...
                {products.map((product) => (
                  ...
                  >
                    <div className="relative aspect-square overflow-hidden m-4 rounded-xl">
                      ...
                      <div className="absolute top-4 right-4 bg-primary-green text-white px-4 py-2 text-sm font-bold shadow-xl rounded-lg border border-white/20">{product.price}€</div>
                    </div>
                          ...
                          >
                            ...
                            {ytId && (
                              <div className="absolute inset-0 flex items-center justify-center">
                                <div className="w-12 h-12 md:w-16 md:h-16 bg-white/20 backdrop-blur-md text-white rounded-full flex items-center justify-center shadow-2xl group-hover:scale-110 group-hover:bg-primary-green transition-all duration-500 border border-white/30">
                    ...
                    {Array.from({ length: totalPages }).map((_, idx) => (
                      <button
                        ...
                        className={`h-2 rounded-full transition-all duration-500 ${
                          galleryIndex === idx ? "w-8 bg-primary-green" : "w-2 bg-white/20 hover:bg-white/50"
                        }`}
                      />
                    ))}
                  </div>
                    ...
                    {user ? (
                      <form onSubmit={handleCommentSubmit} className="space-y-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="overflow-hidden">
                            <p className="text-[9px] font-bold uppercase tracking-widest text-primary-green">Connecté</p>
                            <p className="text-xs text-white/60 truncate">{user.email}</p>
                          </div>
                        </div>
                        ...
                        <textarea
                          ...
                          rows={4}
                          maxLength={300}
                          className="w-full bg-white/[0.04] border border-white/10 rounded-2xl p-4 text-sm outline-none focus:border-primary-green focus:bg-white/10 transition-all text-white placeholder-white/50 font-medium resize-none"
                        />
                        ...
                        <div className="space-y-3 pt-2">
                          <div className="flex items-center justify-between border-b border-white/5 pb-2">
                            ...
                            <div className="flex bg-white/5 rounded-lg p-0.5 border border-white/10">
                              <button
                                ...
                                className={`text-[8px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md transition-all ${commentImgMode === 'upload' ? 'bg-primary-green text-white' : 'text-white/40 hover:text-white'}`}
                              >
                                Importer
                              </button>
                              <button
                                ...
                                className={`text-[8px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md transition-all ${commentImgMode === 'url' ? 'bg-primary-green text-white' : 'text-white/40 hover:text-white'}`}
                              >
                                URL
                              </button>
                            </div>
                          ...
                          {commentImgMode === 'upload' ? (
                            ...
                            <input
                              ...
                              className="w-full bg-white/[0.04] border border-white/10 rounded-2xl p-3 text-[10px] outline-none focus:border-primary-green transition-all text-white placeholder-white/40 font-medium"
                            />
                          )}
                        ...
                        <button
                          ...
                          className="w-full bg-white text-black font-bold uppercase tracking-widest text-[10px] py-4 rounded-2xl hover:bg-primary-green hover:text-white transition-all shadow-xl flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                      ...
                      <div className="text-center py-6 space-y-4">
                        ...
                        <button
                          ...
                          className="w-full bg-primary-green hover:bg-red-600 text-white font-bold uppercase tracking-widest text-[10px] py-3.5 rounded-2xl transition-all shadow-lg"
                        >
                          SE CONNECTER
                        </button>
                      </div>
                          ...
                          <div key={comment.id} className="space-y-4">
                              ...
                              <div className="flex-1 min-w-0 space-y-3">
                                      ...
                                      {isDeleted ? (
                                        ...
                                        <>
                                          ...
                                          {comment.user_email === 'caillatlucas2304@gmail.com' && (
                                            <span className="text-[8px] bg-primary-green/20 text-primary-green px-2 py-0.5 rounded-full border border-primary-green/20 font-bold uppercase tracking-widest">
                                              Admin
                                            </span>
                                          )}
                                        </>
                                ...
                                <div className="flex items-center gap-4 pt-2 border-t border-white/5">
                                  {!isDeleted && (
                                    ...
                                    >
                                      <Heart size={14} className={hasLiked ? "fill-primary-green text-primary-green" : "text-white/40 group-hover/like:text-white transition-colors"} />
                            ...
                            {replyingToId === comment.id && (
                              <div className="bg-[#111111] border border-white/15 rounded-2xl p-5 ml-8 md:ml-12 space-y-4 shadow-2xl">
                                <p className="text-[9px] font-bold uppercase tracking-widest text-white/70">Répondre à <span className="text-primary-green">{replyingToName}</span></p>
                                <form onSubmit={(e) => handleReplySubmit(e, comment.id)} className="space-y-4">
                                  <textarea
                                    ...
                                    rows={2}
                                    maxLength={300}
                                    className="w-full bg-white/[0.04] border border-white/10 rounded-xl p-3 text-xs outline-none focus:border-primary-green focus:bg-white/10 transition-all text-white placeholder-white/50 font-medium resize-none"
                                  />
                                  ...
                                  <div className="flex flex-wrap items-center justify-between gap-3 pt-1 border-t border-white/5">
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div className="flex bg-white/5 rounded-md p-0.5 border border-white/5">
                                        <button
                                          ...
                                          className={`text-[8px] font-bold px-2 py-0.5 rounded transition-all ${replyImgMode === 'upload' ? 'bg-primary-green text-white' : 'text-white/40'}`}
                                        >
                                          Local
                                        </button>
                                        <button
                                          ...
                                          className={`text-[8px] font-bold px-2 py-0.5 rounded transition-all ${replyImgMode === 'url' ? 'bg-primary-green text-white' : 'text-white/40'}`}
                                        >
                                          URL
                                        </button>
                                      </div>
                                    ...
                                    <div className="flex-1 min-w-[120px]">
                                      {replyImgMode === 'upload' ? (
                                        <label className="cursor-pointer text-[9px] font-bold text-primary-green uppercase tracking-widest flex items-center gap-1 hover:text-red-400 justify-end">
                                        ...
                                        <input
                                          ...
                                          placeholder="URL de l'image..."
                                          className="w-full bg-white/[0.04] border border-white/10 rounded-lg p-1.5 text-[9px] outline-none focus:border-primary-green text-white placeholder-white/40 font-medium"
                                        />
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex justify-end gap-2.5 pt-2">
                                    ...
                                    <button
                                      type="submit"
                                      className="px-3.5 py-2 bg-primary-green hover:bg-red-600 text-white rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all"
                                    >
                                      Répondre
                                    </button>
                                  </div>
                                    ...
                                    <div key={reply.id} className="bg-[#0c0c0c]/85 backdrop-blur-2xl border border-white/15 rounded-2xl p-5 md:p-6 flex gap-3.5 md:gap-4 shadow-[inset_0_1px_1px_rgba(255,255,255,0.15),0_8px_32px_rgba(0,0,0,0.5)] relative group hover:bg-[#0c0c0c]/95 transition-all duration-300">
                                      ...
                                      <div className="flex-1 min-w-0 space-y-2">
                                          ...
                                          <div>
                                            <h5 className="text-xs font-bold text-white leading-none flex items-center gap-1.5">
                                              ...
                                              {reply.user_email === 'caillatlucas2304@gmail.com' && (
                                                <span className="text-[7px] bg-primary-green/20 text-primary-green px-1.5 py-0.5 rounded-full border border-primary-green/20 font-bold uppercase tracking-widest">
                                                  Admin
                                                </span>
                                              )}
                                            </h5>
                                        ...
                                        <div className="flex items-center gap-3 pt-1">
                                          ...
                                          >
                                            <Heart size={12} className={hasReplyLiked ? "fill-primary-green text-primary-green" : "text-white/40 group-hover/reply-like:text-white transition-colors"} />
            ...
            <section key="bento" id="bento" className="relative z-10 scroll-mt-28 md:scroll-mt-36">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="md:col-span-2 aspect-square md:aspect-video bg-black/20 backdrop-blur-md border border-white/20 rounded-2xl p-12 flex flex-col justify-between group overflow-hidden relative shadow-2xl hover:bg-black/30 transition-all">
                  <div className="relative z-10"> <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--primary-green)] mb-4">Ma Bio</p> <h3 className="font-serif text-3xl md:text-5xl leading-tight mb-6 text-white">{parseMarkdown(settings.bio || "Exploration créative et solutions techniques.")}</h3> </div>
                  <Socials config={socialsConfig} color={textColor} />
                </div>
                <div className="aspect-square bg-[var(--primary-green)]/90 backdrop-blur-md border border-white/20 rounded-2xl p-10 flex flex-col justify-between text-white relative overflow-hidden group shadow-2xl hover:bg-[var(--primary-green)] transition-all">
